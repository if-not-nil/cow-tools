...
if not util.has("curl") then
	log("curl not available! you need it for this to work")
	os.exit(1)
elseif not util.has("fzf") then
	log("fzf not available! you'll have to do stuff manually")
end
...
local function exec(cmd)
	...
	if not pipe then
		error("can't run command. are you sure it's in your path?")
	end
...
function A:run(path)
	...
	local cat = "cat"
	if util.has("bat") then
		cat = "bat --color=always -p --language=lua"
	end
	local preview_cmd = string.format("%s < <(%s %s preview {})", cat, self_cmd, path)
	local cmd_baked = string.format("echo '%s' | fzf --preview '%s'", t, preview_cmd)
	local cmd, err = exec(cmd_baked)
	panicif(err, err ~= nil)
